...
#include <iostream> // For cout (in handleMouseEvent)

TriangleObject::TriangleObject(uint32_t id, std::pair<int, int> pt1, std::pair<int, int> pt2, std::pair<int, int> pt3, bool isDisplayed, COLORREF color)
...
void TriangleObject::handleMouseEvent(const MOUSEMSG &msg, string *result) {
...
bool TriangleObject::isOverlapping(int x, int y) const {
    int x1 = this->pt1.first * scaleFactor;
    int y1 = this->pt1.second * scaleFactor;
    int x2 = this->pt2.first * scaleFactor;
    int y2 = this->pt2.second * scaleFactor;
